...
def emit_python(
    ...
    sel_map = {
        method_name(f["name"], f.get("discriminator")): f["selector"]
        for f in ir["functions"]
    }
    topic_map = {
        method_name(e["name"], e.get("discriminator")): e["topic_id"]
        for e in ir.get("events", [])
    }

    lines: List[str] = []
    lines.append(
        "# This file was generated by Animica SDK codegen (Python). Do not edit by hand.\n"
    )
    ...
    lines.append(
        f"SELECTORS: dict[str, str] = {json.dumps(sel_map, separators=(',',':'))}\n"
    )
    lines.append(
        f"TOPICS: dict[str, str] = {json.dumps(topic_map, separators=(',',':'))}\n"
    )
    lines.append("\n")
    ...
    for f in ir["functions"]:
        ...
        params = ", ".join(
            f"{py_ident(p['name'])}: {py_type(p['type'])}" for p in f["inputs"]
        )
        if params:
            params = ", " + params
        ...
        lines.append(
            f"        return self.{inv}({json.dumps(f['name'])}, [{arg_list}])\n\n"
        )
    return "".join(lines)
...
def emit_typescript(
    ...
    lines: List[str] = []
    lines.append(
        "// This file was generated by Animica SDK codegen (TypeScript). Do not edit by hand.\n"
    )
    ...
    lines.append(
        f"export const SELECTORS = {json.dumps(sel_map, separators=(',',':'))} as const;\n"
    )
    lines.append(
        f"export const TOPICS = {json.dumps(topic_map, separators=(',',':'))} as const;\n\n"
    )
    ...
    for f in ir["functions"]:
        ...
        params = ", ".join(
            f"{ts_ident(p['name'])}: {ts_type(p['type'])}" for p in f["inputs"]
        )
        is_view = f["state_mutability"] in ("view", "pure")
        if is_view:
            if len(f["outputs"]) == 0:
...
def emit_rust(
    ...
    lines.append(
        'pub fn abi_value() -> Json { serde_json::from_str(ABI_JSON).expect("ABI_JSON") }\n\n'
    )
    lines.append(
    ...
    lines.append(
        f"  pub fn new(address: {address_type}, opts: Option<{base_path}::Options>) -> Self {{\n"
    )
    lines.append(
        f"    let inner = {base_class}::new(address, abi_value(), opts);\n    Self {{ inner }}\n  }}\n\n"
    )
    lines.append("  #[inline] pub fn address(&self) -> &str { self.inner.address() }\n")
    lines.append(
        f"  #[inline] pub fn set_signer(&mut self, signer: Option<{base_path}::Signer>) {{ self.inner.set_signer(signer) }}\n\n"
    )
    for f in ir["functions"]:
        mname = method_name(f["name"], f.get("discriminator"))
        params_sig = ", ".join(
            f"{rs_ident(p['name'])}: {rs_type(p['type'])}" for p in f["inputs"]
        )
        is_view = f["state_mutability"] in ("view", "pure")
        if is_view:
            if len(f["outputs"]) == 0:
        ...
        lines.append(
            f"  /// {f['name']} {f['state_mutability']} | Selector: {f['selector']}\n"
        )
        lines.append(
            f"  pub async fn {rs_ident(mname)}(&self, {params_sig}) -> Result<{ret_ty}, {base_path}::Error> {{\n"
        )
        lines.append(
            f"    self.inner.{inv}::<{ret_ty}>({rs_string(f['name'])}, serde_json::json!([{args_vec}])).await\n"
        )
        lines.append("  }\n\n")
    lines.append(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    p.add_argument(
        "--lang", required=True, choices=["py", "ts", "rs"], help="Target language"
    )
    p.add_argument(
        "--abi", required=True, help="Path to normalized ABI IR JSON (or '-' for stdin)"
    )
    ...
    p.add_argument(
        ...
        dest="class_name",
        default=None,
        help="Class/struct name (default inferred or 'ContractClient')",
    )
    p.add_argument(
        "--file",
        dest="file_name",
    ...
    if opts.lang == "py":
        code = emit_python(
            ir, cls, base_import=opts.py_base_import, base_class=opts.py_base_class
        )
        fname = opts.file_name or f"{snake(cls)}.py"
    elif opts.lang == "ts":
        code = emit_typescript(
            ir, cls, base_import=opts.ts_base_import, base_class=opts.ts_base_class
        )
        ...
        code = emit_rust(
            ir,
            cls,
            base_path=opts.rs_base_path,
            base_class=opts.rs_base_class,
            address_type=opts.rs_address_type,
        )
