...
def build_manifest(website_dir: Path, version: str, generated_at: str, architecture: str) -> dict[str, object]:
    manifest: dict[str, object] = {
        "version": version,
        "generated_at": generated_at,
    }

    windows = build_windows_section(website_dir, version, architecture)
    linux = build_linux_section(website_dir, version, architecture)

    if windows is not None:
        manifest["windows"] = windows
    if linux is not None:
        manifest["linux"] = linux
    if "windows" not in manifest and "linux" not in manifest:
...
def main() -> int:
    ...
    try:
        manifest = build_manifest(website_dir, args.version, args.generated_at, args.architecture)
