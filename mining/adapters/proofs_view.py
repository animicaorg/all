...
try:
    ...
    from proofs.cbor import decode_envelope as _dec1  # type: ignore[attr-defined]
...
try:
    from proofs.registry import get_verifier as _get_verifier  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    _get_verifier = None
...
try:
    from proofs.hashshare import verify as _ver_hash  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    pass
try:
    ...
    pass
try:
    from proofs.quantum import verify as _ver_quantum  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    pass
try:
    from proofs.storage import verify as _ver_storage  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    pass
try:
...
try:
    ...
    try:
        from proofs.policy_adapter import map_metrics_to_psi as _metrics_to_psi  # type: ignore[attr-defined]
    except Exception:  # noqa: BLE001
...
try:
    from proofs.receipts import build_receipt as _build_receipt  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    _build_receipt = None

_compute_nullifier: Optional[Callable[[str, Dict[str, Any]], bytes]] = None
try:
    from proofs.nullifiers import compute_nullifier as _compute_nullifier  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    _compute_nullifier = None

# Some utility hashing if needed for fallback nullifiers
try:
    from proofs.utils.hash import sha3_256 as _sha3_256  # type: ignore[attr-defined]
except Exception:  # noqa: BLE001
    try:
        from core.utils.hash import sha3_256 as _sha3_256  # type: ignore[attr-defined]
    except Exception:  # noqa: BLE001
        _sha3_256 = None
...
class ProofsViewAdapter:
    ...
    # ---------------------------- public API ----------------------------
    def verify(self, envelope: Envelope, *, context: Optional[Dict[str, Any]] = None) -> VerifyResult:
        """
    ...
    def verify_batch(
        ...
        *,
        ...
        return {"items": items, "psi_inputs": psi_inputs, "ok": True, "failed_index": None, "error": None}
    ...
    def _ensure_envelope_dict(self, envelope: Envelope) -> Dict[str, Any]:
        if isinstance(envelope, (bytes, bytearray, memoryview)):
            if _decode_envelope is None:
                raise ProofsViewError("CBOR decoder is unavailable and envelope is bytes")
        ...
        *,
        ...
        if _get_verifier is not None:
            try:
                ...
                raise ProofsViewError(f"Verifier via registry failed for {type_id}: {e}") from e
        ...
        try:
            ...
            raise ProofsViewError(f"Direct verifier call failed for {type_id}: {e}") from e

        # 3) Last-resort heuristic validation (non-consensus; DEV ONLY)
        log.warning("No verifier found for type_id=%s; using heuristic DEV checks", type_id)
    ...
    def _map_metrics_to_psi(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        if _metrics_to_psi is not None:
            try:
                ...
                log.warning("metrics_to_psi failed; using heuristic mapping", extra={"err": str(e)})
        ...
        if kind == "quantum":
            ...
            traps = float(metrics.get("traps_ratio", 0.0))
            qos = float(metrics.get("qos", 0.0))
            return {"kind": "quantum", "psi": 0.002 * units * _clip01(traps) * _clip01(qos)}
        if kind == "storage":
            qos = float(metrics.get("qos", 0.0))
    ...
    def _build_receipt_safe(self, type_id: str, env: Dict[str, Any], metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _heuristic_verify(self, type_id: str, body: Any) -> Dict[str, Any]:
        ...
        if type_id in ("quantum", "quantum_v1"):
            ...
            return {"kind": "quantum", "quantum_units": units, "traps_ratio": traps, "qos": qos}
...
def _cheap_json_like(obj: Any) -> str:
    ...
    if isinstance(obj, dict):
        ...
        items = ",".join(f"{repr(k)}:{_cheap_json_like(obj[k])}" for k in sorted(obj.keys()))
        return "{" + items + "}"
