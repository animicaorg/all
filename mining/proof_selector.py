...
def select_proofs(
    ...
    **_kw: Any,
    ...
    policy = policy or {}
    caps = caps or per_type_caps or policy.get("per_type_caps") or policy.get("per_type") or {}
    # normalize dicts like {"hash":{"cap":2}} -> {"hash":2}
    norm_caps: Dict[str, int] = {}
    for t, v in caps.items():
        if isinstance(v, dict) and "cap" in v:
            try: norm_caps[t] = int(v["cap"])
            except: norm_caps[t] = 0
        else:
            try: norm_caps[t] = int(v)
            except: norm_caps[t] = 0
    caps = norm_caps
    ...
    gcap = (gamma_cap if gamma_cap is not None else
            Gamma_cap if Gamma_cap is not None else
            total_cap if total_cap is not None else
            total_Gamma if total_Gamma is not None else
            policy.get("gamma_cap"))

    # escort/fairness present?
    q = (escort_q if escort_q is not None else
         fairness_q if fairness_q is not None else
         diversity_q if diversity_q is not None else
         policy.get("escort_q"))
        ...
        if apply_fairness and sum(counts.values()) > 0:
            ...
            if frac_t > q:
                ...
                for other in ordered:
                    ot = _ptype(other)
                    if ot == t: continue
                    if counts.get(ot, 0) >= caps.get(ot, 1<<30): continue
                    if gcap is not None and (gamma_total + score(other)) > (gcap + 1e-12): continue
                    alt_ok = True
                    break
                if alt_ok:
                    continue
