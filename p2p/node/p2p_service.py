...
class P2PService:
    ...
    def __init__(
        ...
        *,
        ...
        verifier_ips_env = os.environ.get("ANIMICA_P2P_VERIFIER_SEED_IPS", "62.169.17.132,82.208.20.209,144.126.133.21")
    ...
    def _update_peer_head(
        ...
        if height <= current:
            self._update_peer_head_table(
                peer, height=int(current), source="peer_head", head_hash=head_hash
            )
            return
    ...
    def _reset_from_highest_next_height(self, *, reason: str) -> None:
        ...
        if best_height is not None:
            self._sync_target_height = int(best_height)
