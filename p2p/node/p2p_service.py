...
class P2PService:
    ...
    def __init__(
        ...
        *,
        ...
        tick_ms = float(_env_value("SYNC_TICK_MS", "ANIMICA_SYNC_TICK_MS", default="50") or 50)
        ...
        self._sync_max_inflight_per_peer = int(
            os.environ.get("ANIMICA_P2P_SYNC_INFLIGHT_PER_PEER", "128") or 128
        )
        self._sync_headers_batch = int(
            os.environ.get("ANIMICA_P2P_SYNC_HEADERS_BATCH", "2048") or 2048
        )
        self._sync_request_timeout = float(
            os.environ.get("ANIMICA_P2P_SYNC_TIMEOUT", "6.0") or 6.0
        )
    ...
    async def force_sync(self) -> dict[str, Any]:
        self._request_sync(reason="force_sync")
        return await self._sync_once(force=True)

    def _request_sync(self, *, reason: str) -> None:
        self._sync_requested = True
        self._sync_requested_at = time.time()
        self._sync_wakeup.set()
        log.debug("Sync requested", extra={"reason": reason})
    ...
    async def force_sync_with_cache(
        ...
        self._request_sync(reason="force_sync_with_cache")
            ...
            if ok and not local:
                ...
                if instant_blocks_enabled:
                    try:
                        ...
                        pass  # Best-effort, don't fail tx admission
