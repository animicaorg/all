...
def build_host_for_id(service_id: str, env: Dict[str, str], root_domain: Optional[str]) -> Optional[str]:
...
def gather_entry(service_id: str, env: Dict[str, str], root_domain: Optional[str]) -> Optional[Dict[str, object]]:
...
def render_yaml(block: Dict[str, object]) -> str:
    ...
    lines: List[str] = []
    lines.append("# Generated by ops/scripts/render_ingress_values.py on " +
...
def main() -> int:
    ...
    ap.add_argument("--env", default="ops/.env", help="Path to .env (default: ops/.env)")
    ap.add_argument("--out", default="", help="Write YAML to this file (default: stdout)")
    args = ap.parse_args()
    ...
    for sid in ids:
        ...
        hosts.append({
            ...
            **({"tlsSecretName": entry["tls_secret"]} if entry["tls_secret"] else {}),
        })

    if not hosts:
        print("ERROR: no hosts discovered (set ROOT_DOMAIN/BASE_DOMAIN or DOMAIN_* entries).", file=sys.stderr)
        return 3
