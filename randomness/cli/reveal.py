"""
randomness.cli.reveal
---------------------

Thin CLI for submitting a reveal to the randomness service.

Usage:
  omni rand reveal --salt 0xdeadbeef --payload 0x0123
  # Optionally specify the address (otherwise taken from env):
  omni rand reveal --address anim1... --salt 0x.. --payload 0x..
...
app = typer.Typer(
    name="omni-rand-reveal",
    help="Submit a reveal for the current randomness round.",
...
def _is_hex(s: str) -> bool:
    return s.startswith(("0x", "0X")) and all(c in "0123456789abcdefABCDEF" for c in s[2:]) and (len(s) % 2 == 0)


@app.command("reveal")
def cmd_reveal(
    salt: str = typer.Option(..., "--salt", "-s", help="0x-hex salt used at commit time."),
    payload: str = typer.Option(..., "--payload", "-p", help="0x-hex payload revealed to match the commitment."),
    address: Optional[str] = typer.Option(None, "--address", "-a", help="Sender address (defaults to env OMNI_ADDRESS/ANIMICA_ADDRESS)."),
    rpc: str = typer.Option(_DEFAULT_RPC, "--rpc", help=f"JSON-RPC endpoint (default: {_DEFAULT_RPC})"),
) -> None:
    """
    Submit a reveal for the currently reveal-open round.

    Server verifies that H(domain|address|salt|payload) matches a prior commitment.
    """
    addr = address or _DEFAULT_ADDR
    if not addr:
        raise SystemExit("Missing --address (and OMNI_ADDRESS/ANIMICA_ADDRESS not set).")
    if not _is_hex(salt):
...
def main() -> None:  # pragma: no cover
    try:
        app(standalone_mode=False, prog_name="omni rand reveal")
