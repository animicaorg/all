...
print(f'  MAINNET_CHAIN_ID = {MAINNET_CHAIN_ID} (expected: 0)')
print(f'  TESTNET_CHAIN_ID = {TESTNET_CHAIN_ID} (expected: 1)')
print(f'  DEVNET_CHAIN_ID = {DEVNET_CHAIN_ID} (expected: 1337)')
assert MAINNET_CHAIN_ID == 0, 'Mainnet should be 0'
assert TESTNET_CHAIN_ID == 1, 'Testnet should be 1'
...
if grep -q '"animica:1":' spec/params.yaml; then
    echo "  ✓ Found animica:1 for testnet"
else
    echo "  ✗ Missing animica:1 in params.yaml"
    exit 1
fi

if grep -q '"animica:0":' spec/params.yaml; then
    echo "  ✓ Found animica:0 for mainnet"
else
    echo "  ✗ Missing animica:0 in params.yaml"
    exit 1
fi
echo ""
...
if [ "$testnet_id" = "1" ]; then
    echo "  ✓ Testnet genesis has chainId=1"
else
    echo "  ✗ Testnet genesis has chainId=$testnet_id (expected 1)"
    exit 1
fi
echo ""

echo "5. Checking docker-compose.testnet.yml..."
if grep -q 'ANIMICA_CHAIN_ID:-1}' ops/docker/docker-compose.testnet.yml; then
    echo "  ✓ Docker Compose uses chain_id=1 default"
else
    echo "  ✗ Docker Compose not using chain_id=1"
    exit 1
fi
echo ""
