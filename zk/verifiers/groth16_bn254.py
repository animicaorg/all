...
def load_vk(vk_json: dict) -> VerifyingKey:
    ...
    # Basic sanity
    if not (_on_curve_g1(alpha1) and _on_curve_g2(beta2) and _on_curve_g2(gamma2) and _on_curve_g2(delta2)):
        raise ValueError("VK points are not on curve")
    for P in ic_pts:
        if not _on_curve_g1(P):
            raise ValueError("IC point not on G1 curve")

    return VerifyingKey(alpha1=alpha1, beta2=beta2, gamma2=gamma2, delta2=delta2, IC=ic_pts)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    if len(sys.argv) == 3:
        ...
        ok = verify_groth16(vk, proof_obj["proof"] if "proof" in proof_obj else proof_obj, inputs)
        print("verify ->", ok)
    else:
        print("Run with: python zk/verifiers/groth16_bn254.py path/to/vk.json path/to/proof.json")
