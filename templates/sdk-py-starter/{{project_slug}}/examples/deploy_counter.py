...
def load_artifacts(*, manifest_path: Optional[Path], code_path: Optional[Path], package_path: Optional[Path]) -> DeployArtifacts:
    """
    ...
    if package_path:
        ...
        if "code_hex" in pkg:
            ...
            code_hex = code_hex[2:] if isinstance(code_hex, str) and code_hex.startswith("0x") else code_hex
    ...
    return DeployArtifacts(manifest=_read_json(manifest_path), code=_read_bytes(code_path))
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--rpc", default=os.getenv("ANIMICA_RPC_URL", "http://127.0.0.1:8545"), help="HTTP RPC URL")
    ap.add_argument("--chain", type=int, default=_env_int("ANIMICA_CHAIN_ID", 1337), help="Chain ID")
    ap.add_argument("--mnemonic", default=os.getenv("DEPLOYER_MNEMONIC"), help="Deployer mnemonic (or set DEPLOYER_MNEMONIC)")
    ap.add_argument("--ws", default=None, help="Optional WebSocket URL (to stream receipt faster)")
    ap.add_argument("--alg", default="dilithium3", choices=["dilithium3", "sphincs_shake_128s"], help="Signature algorithm")
    ap.add_argument("--nonce", type=int, default=None, help="Explicit sender nonce (optional)")
    ...
    # artifacts
    ap.add_argument("--manifest", type=Path, default=Path("./artifacts/counter/manifest.json"), help="Path to manifest.json")
    ap.add_argument("--code", type=Path, default=Path("./artifacts/counter/code.ir"), help="Path to compiled code/IR blob")
    ap.add_argument("--package", type=Path, default=None, help="Single JSON package containing {manifest, code_hex|code_base64}")
    args = ap.parse_args(argv)

    if not args.mnemonic:
        print("error: missing mnemonic; set --mnemonic or DEPLOYER_MNEMONIC", file=sys.stderr)
        return 2
    ...
    # Await receipt
    try:
        receipt = tx_send.wait_for_receipt(http, tx_hash, timeout=60.0, poll_interval=1.0)
    except Exception as e:
    ...
    try:
        ...
        result = http.call("state.call", [{
            "to": contract_addr,
            "data": {"method": "get", "args": []},  # ABI-encoded by the node's minimal VM shim, if supported
        }])
