...
from typing import Any, AsyncIterator, Dict, Iterable, List, Mapping, Optional, Tuple, Union, cast
...
class RpcError(Exception):
    def __init__(self, code: int, message: str, data: Any = None) -> None:
        ...
        self.data = data
        super().__init__(f"RPC error {code}: {message}" + (f" | data={data}" if data is not None else ""))
...
def _ok(resp_json: Json) -> Any:
    if "error" in resp_json and resp_json["error"]:
        ...
        raise RpcError(int(err.get("code", -1)), str(err.get("message", "unknown error")), err.get("data"))
...
class JsonRpcClient:
    ...
    async def __aenter__(self) -> "JsonRpcClient":
        self._http = httpx.AsyncClient(base_url=self.cfg.rpc_url, timeout=self.cfg.http_timeout_s)
    ...
    async def batch(self, calls: Iterable[Tuple[str, Params]]) -> List[Any]:
        ...
        for method, params in calls:
            ...
            items.append({"jsonrpc": "2.0", "id": ident, "method": method, "params": params or []})
            order.append(ident)
        ...
        for attempt in range(1, self.cfg.http_retries + 1):
            try:
                ...
                if isinstance(payload, list):
                    return data
                return data
            except (httpx.ConnectError, httpx.ReadTimeout, httpx.RemoteProtocolError, httpx.WriteError, httpx.HTTPStatusError) as e:  # noqa: E501
                last_exc = e
                sleep_s = min(self.cfg.ws_backoff_initial_s * (2 ** (attempt - 1)), self.cfg.ws_backoff_max_s)
                self._log.warning("RPC POST failed (attempt %d/%d): %s; sleeping %.2fs", attempt, self.cfg.http_retries, e, sleep_s)  # noqa: E501
                await asyncio.sleep(sleep_s)
            except json.JSONDecodeError as e:
                raise RpcError(-1, f"Invalid JSON in response: {e}") from e

        assert last_exc is not None
        raise RpcError(-1, f"RPC POST failed after {self.cfg.http_retries} attempts: {last_exc}")
    ...
    async def get_block_by_number(self, number: Union[int, str], full_txs: bool = False) -> Json:
    ...
    async def get_block_by_hash(self, block_hash: str, full_txs: bool = False) -> Json:
        return cast(Json, await self.call(ETH_GET_BLOCK_BY_HASH, [block_hash, full_txs]))

    async def get_tx(self, tx_hash: str) -> Optional[Json]:
        return cast(Optional[Json], await self.call(ETH_GET_TRANSACTION_BY_HASH, [tx_hash]))
    ...
    async def get_block_range(
        self, start: int, end_inclusive: int, full_txs: bool = False, max_batch: Optional[int] = None
    ) -> List[Json]:
        """
        ...
        while cur <= end_inclusive:
            ...
            batch_calls = [
                (ETH_GET_BLOCK_BY_NUMBER, [_int_to_hex(n), full_txs]) for n in range(cur, chunk_end + 1)
            ]
    ...
    async def _ws_stream(self, topic: str, params: Optional[Union[Mapping[str, Any], List[Any]]] = None) -> AsyncIterator[Any]:  # noqa: E501
        """
        ...
        while True:
            try:
                ...
                self._log.warning("WS stream error (%s). Reconnecting in %.2fs ...", e, backoff)
    ...
    async def _ws_session(
        ...
        async with websockets.connect(self.cfg.ws_url) as ws:  # type: ignore[arg-type]
            ...
            hb_task = asyncio.create_task(self._ws_heartbeat(ws, self.cfg.ws_heartbeat_s))
            try:
                while True:
                    raw = await ws.recv()
                    msg = json.loads(raw)
    ...
    async def _ws_subscribe(self, ws: WebSocketClientProtocol, subscribe_params: List[Any]) -> str:
        """
        Send `eth_subscribe` request and wait for the result (subscription id).
        """
        req_id = uuid.uuid4().hex
        req = {"jsonrpc": "2.0", "id": req_id, "method": ETH_SUBSCRIBE, "params": subscribe_params}
