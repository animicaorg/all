...
def test_manifest_has_required_functions():
    ...
    assert "inc" in funcs, f'"inc" function not found in manifest ABI. Found: {sorted(funcs)}'
    assert "get" in funcs, f'"get" function not found in manifest ABI. Found: {sorted(funcs)}'

    # Nice-to-have: names are unique
    assert len(funcs) == len(set(funcs)), "Duplicate function names in ABI (should be unique)."
...
def test_source_uses_deterministic_subset():
    ...
    banned_modules = [
        r"\bimport\s+os\b", r"\bfrom\s+os\s+import\b",
        r"\bimport\s+sys\b", r"\bfrom\s+sys\s+import\b",
        r"\bimport\s+time\b", r"\bfrom\s+time\s+import\b",
        r"\bimport\s+socket\b", r"\bfrom\s+socket\s+import\b",
        r"\bimport\s+subprocess\b", r"\bfrom\s+subprocess\s+import\b",
        r"\bimport\s+threading\b", r"\bfrom\s+threading\s+import\b",
        r"\bimport\s+multiprocessing\b", r"\bfrom\s+multiprocessing\s+import\b",
        r"\bimport\s+asyncio\b", r"\bfrom\s+asyncio\s+import\b",
        r"\bimport\s+requests\b", r"\bfrom\s+requests\s+import\b",
        r"\bimport\s+urllib\b", r"\bfrom\s+urllib\s+import\b",
        r"\bimport\s+http\b", r"\bfrom\s+http\s+import\b",
    ]
    banned_primitives = [
        r"\beval\s*\(",
        r"\bexec\s*\(",
...
def test_build_script_produces_artifacts(tmp_path):
    ...
    if not json_candidates:
        pytest.skip("Build produced no JSON artifacts to inspect (acceptable for minimal templates).")
    ...
    for jp in json_candidates:
        ...
        if isinstance(data, dict):
            ...
            elif isinstance(data.get("result"), dict) and isinstance(data["result"].get("codeHash"), str):
                candidate = data["result"]["codeHash"]

        if candidate:
            found_any_hash = True
            assert candidate.startswith("0x") and len(candidate) >= 10, (
                f"Suspicious codeHash value in {jp}: {candidate!r}"
            )
            break

    if not found_any_hash:
        pytest.skip("No codeHash field found in build JSON artifacts (acceptable for minimal templates).")
...
def test_vm_toolchain_present():
    ...
    try:
        ...
        pytest.skip("Python not found in PATH (unexpected), skipping optional VM check.")
