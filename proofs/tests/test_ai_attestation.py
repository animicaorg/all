...
def _call_verifier(vf: Verifier, evidence: bytes, roots_dir: Optional[Path] = None, **kw) -> Tuple[bool, Dict[str, Any]]:
    """
...
def test_sgx_quote_parse_and_verify_or_parse_only():
    ...
    if det:
        found_identity = any(k in det for k in ("mrenclave", "mrsigner", "tcb", "tee", "qe", "quote"))
        assert found_identity, f"Expected identity-ish fields in details, got keys={list(det.keys())[:6]}"
...
def test_sev_snp_report_parse_and_verify_or_parse_only():
    ...
    assert ok or det.get("parsed") is True
    if det:
        found_identity = any(k in det for k in ("chip_id", "family_id", "report", "policy", "tcb_version"))
        assert found_identity, f"Expected SEV/SNP fields, got keys={list(det.keys())[:6]}"
...
def test_cca_token_parse_and_verify_or_parse_only():
    ...
    if det:
        ...
        expect_any = ("platform-hash", "realm-hash", "realm-svn", "nonce", "profile", "claims", "cose")
        assert any(k in det for k in expect_any), f"Expected CCA claims; got keys={list(det.keys())[:6]}"
...
def test_cca_rejects_garbage():
    vf = _find_verifier(cca)
    with pytest.raises(AttestationError):
        _call_verifier(vf, b"\xD8\x50\xA1\x01", roots_dir=VENDOR)  # malformed tiny CBOR/COSE
