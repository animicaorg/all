...
def _samples_for_target_fn() -> Optional[Callable[[int, int, float], int]]:
    names = ("samples_for_target", "required_samples", "samples_needed_for_target", "sample_count_for_target")
    for nm in names:
        if _has(nm):
            fn = _get(nm)
            return lambda N, bad, target, _fn=fn: int(_try_call(_fn, N=N, bad=bad, target=target))
    return None
...
def test_upper_bound_sandwich_vs_exact_and_with_replacement(N: int, bad: int, s: int):
    ...
    if pf_exact_mod is not None:
        ...
        assert abs(val - pf_exact_local) <= 1e-12, "module exact probability must match hypergeometric product"
...
def test_monotone_in_samples(N: int, bad: int):
    ...
    for a, b in zip(seq, seq[1:]):
        assert b <= a + 1e-12, f"p_fail must be non-increasing in samples (got {a} -> {b})"
...
def test_monotone_in_bad_fraction(N: int, s: int):
    ...
    for a, b in zip(vals, vals[1:]):
        assert b <= a + 1e-12, f"p_fail must decrease as adversarial fraction increases (got {a} -> {b})"
...
@pytest.mark.parametrize("N,bad,target", [
...
def test_samples_for_target_is_sound_and_close(N: int, bad: int, target: float):
    ...
    assert s <= s_exact + 8, "Returned samples are unexpectedly loose vs exact requirement (slack 8)"
