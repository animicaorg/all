...
def compact_to_target(bits: int) -> int:
    exp = bits >> 24; mant = bits & 0xffffff; return mant << (8 * (exp - 3))
...
def pure(limit=1_000_000):
    start = time.time(); found = 0; base = random.randrange(0, 2**32-1)
...
try:
    ...
    for name in ("search","scan_range","cpu_search","search_cpu"):
        if hasattr(mh, name): func = getattr(mh, name); break
    if func:
        print(f"[repo] using mining.hash_search.{func.__name__}")
        ret = func(header_prefix, 0, 1_000_000, target)  # best-effort signature
        out = list(ret) if ret is not None and not isinstance(ret, list) else (ret or [])
