...
def mint_bar_footer(slide, text="Animica — animica.dev"):
    ...
    bar_h = 0.28
    bar = slide.shapes.add_shape(MSO.RECTANGLE, Inches(0), prs.slide_height - Inches(bar_h), prs.slide_width, Inches(bar_h))
    bar.fill.solid()
    bar.fill.fore_color.rgb = MINT
    bar.line.fill.background()

    tb = slide.shapes.add_textbox(Inches(0.35), prs.slide_height - Inches(bar_h) + Inches(0.06), Inches(6), Inches(0.4))
    tf = tb.text_frame
    tf.text = text
    p = tf.paragraphs[0]
    p.font.size = Pt(12)
...
def pill(slide, x=1.3, y=0.6, w=1.7, h=0.5, text="ANIMICA"):
    sh = slide.shapes.add_shape(MSO.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(w), Inches(h))
    sh.fill.solid(); sh.fill.fore_color.rgb = MINT
    sh.line.fill.background()
    tf = sh.text_frame
    tf.text = text
    p = tf.paragraphs[0]
    p.font.size = Pt(18); p.font.bold = True; p.font.color.rgb = INK
...
def three_cols(slide, cols, start_y=2.2):
    ...
    for title, body in cols:
        hb = slide.shapes.add_textbox(Inches(x), Inches(start_y), Inches(col_w), Inches(0.6))
        ht = hb.text_frame; ht.clear()
        p = ht.paragraphs[0]
        p.text = title; p.font.bold = True; p.font.size = Pt(28); p.font.color.rgb = SLATE

        bb = slide.shapes.add_textbox(Inches(x), Inches(start_y+0.7), Inches(col_w), Inches(2.4))
        bt = bb.text_frame; bt.clear()
        for i, line in enumerate(body.split("\n")):
            p = bt.paragraphs[0] if i == 0 else bt.add_paragraph()
            p.text = line; p.font.size = Pt(22); p.font.color.rgb = S600
        x += col_w + gap
...
subtitle_shape(s, "Deterministic Python VM · Post-Quantum Security · Useful-Work Mining", y=2.6)
...
ring.fill.background(); ring.line.width = Pt(4); ring.line.color.rgb = MINT
spark = s.shapes.add_shape(MSO.OVAL, Inches(0.85), Inches(6.55), Inches(0.14), Inches(0.14))
spark.fill.solid(); spark.fill.fore_color.rgb = MINT; spark.line.fill.background()
mint_bar_footer(s)
...
three_cols(s, [
    ("Deterministic VM", "Python-VM with a constrained stdlib\nCanonical hashing & ABI\nGas-metered I/O"),
    ("Post-Quantum First", "Dilithium3 signatures\nKyber-768 KEX (rotatable)\nRotation policies in governance"),
    ("Governance, Formalized", "Schemas & registries in-repo\nParam bounds & CI checks\nSafe, staged upgrades"),
])
mint_bar_footer(s)
...
subtitle_shape(s, "Focus: builders & security-sensitive apps (fintech, RWA, compliance).", y=4.4, size=22, color=S600)
mint_bar_footer(s)
...
subtitle_shape(s, "Advisors: cryptography, governance, infra.", y=4.4, size=22, color=S600)
mint_bar_footer(s)
