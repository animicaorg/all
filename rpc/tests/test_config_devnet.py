...
def test_devnet_config_uses_genesis_chain_id(monkeypatch, tmp_path):
    genesis_src = Path(__file__).resolve().parents[2] / "genesis" / "genesis.sample.devnet.json"
    ...
    monkeypatch.setenv("ANIMICA_RPC_DB_URI", "sqlite:////${HOME}/animica/devnet/chain.db")
