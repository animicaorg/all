...
@pytest.mark.parametrize(
    "exc, expected_code",
    [
        (mp_errors.FeeTooLow(offered_gas_price_wei=1, min_required_wei=10), rpc_errors.AnimicaCode.FEE_TOO_LOW),
        (mp_errors.NonceGap(expected_nonce=2, got_nonce=5), rpc_errors.AnimicaCode.NONCE_TOO_LOW),
        (mp_errors.Oversize(size_bytes=2048, max_bytes=1024), rpc_errors.AnimicaCode.TX_TOO_LARGE),
    ],
)
def test_mempool_errors_are_preserved_in_rpc(exc: mp_errors.MempoolError, expected_code: rpc_errors.AnimicaCode):
    """
