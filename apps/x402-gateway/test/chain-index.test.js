 ...
 *
 *   chain_address_history  POST /x402/chain/address-history  $0.05
...
test('history: 402 then a paid page with the documented derivation and exact big amounts', async () => {
  ...
  try {
    ...
    assert.equal(offer.accepts[0].maxAmountRequired, '50000'); // $0.05 in USDC atomic units
    ...
    assert.equal(b.payment.price_usd, '0.05');
...
test('balances: one batched RPC, deduped, BigInt-exact decimal strings', async () => {
  ...
  try {
    ...
    assert.equal(first.status, 402);
    assert.equal(JSON.parse(first.text).accepts[0].maxAmountRequired, '20000'); // $0.02
    assert.equal(paid.status, 200);
...
test('catalog: both chain products publish price, caps and an input schema', async () => {
  ...
  try {
    ...
    assert.equal(hist.price, '0.05');
    assert.equal(hist.price_atomic, '50000');
    ...
    assert.equal(bal.price, '0.02');
    assert.equal(bal.price_atomic, '20000');
