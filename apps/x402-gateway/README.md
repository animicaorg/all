...
| no third-party settlement dependency | `honesty-guards` — no third-party facilitator URL, no facilitator wiring naming an external provider and no external payment SDK anywhere in src, nginx, systemd, bin, env template or docs; `mode` defaults to `self`; `remote` without an explicit URL fails closed (`evm`). Read-only discovery endpoints (`…/discovery/resources`) are the one permitted external URL, and a second guard asserts the Mesh index only ever GETs them |
