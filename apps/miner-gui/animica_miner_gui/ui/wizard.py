...
1. Network selection (mainnet/testnet/devnet) - always runs local node
2. Local node configuration
3. Wallet/payout address setup
4. Device detection and selection
5. Performance presets
6. Summary and start mining

Note: This GUI ONLY runs a local node. Remote RPC connections are not supported.
"""
...
class NetworkSelectionPage(QWizardPage):
    ...
    Note: This GUI always runs a local node. You're selecting which network
    the local node should connect to.
    """
    
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setTitle("Network Selection")
        self.setSubTitle("Choose which Animica network your local node will connect to")
        
        layout = QVBoxLayout()
        
        # Info message
        info_label = QLabel(
            "ℹ️ This application runs a local Animica node on your computer. "
            "Select which network your node should connect to:"
        )
        ...
        self.mainnet_radio = QRadioButton("Mainnet (production network)")
        self.testnet_radio = QRadioButton("Testnet (testing with real network conditions)")
        self.devnet_radio = QRadioButton("Devnet (local development and testing)")
        ...
        desc_label.setStyleSheet("color: gray; margin-top: 8px; font-size: 11px;")
        layout.addWidget(desc_label)
        
        def update_description():
            if self.mainnet_radio.isChecked():
                desc_label.setText(
                    "Your local node will sync with the Mainnet network. "
                    "This is the production network with real value."
                )
            elif self.testnet_radio.isChecked():
                desc_label.setText(
                    "Your local node will sync with the Testnet network. "
                    "This network uses test tokens with no real value."
                )
            else:
                desc_label.setText(
                    "Your local node will run in local development mode. "
                    "Perfect for testing without network connectivity."
                )
        
        # Connect signals
        self.mainnet_radio.toggled.connect(update_description)
        self.testnet_radio.toggled.connect(update_description)
        self.devnet_radio.toggled.connect(update_description)
        update_description()
...
class LocalNodeConfigPage(QWizardPage):
    """Local node configuration page.
    
    Replaces the old RPCConfigPage with local-node-specific settings.
    """
    
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setTitle("Local Node Configuration")
        self.setSubTitle("Configure your local Animica node")
        ...
        self.network_display = QLabel()
        self.network_display.setStyleSheet("font-weight: bold;")
        network_layout.addWidget(self.network_display)
        network_layout.addStretch()
        layout.addLayout(network_layout)
        
        # Info about local node
        info_label = QLabel(
            "✓ Your local node will run on 127.0.0.1 (localhost only)\n"
            "✓ The node will be started automatically when you launch the miner\n"
            "✓ All data will be stored in ~/.animica/"
        )
        info_label.setWordWrap(True)
        info_label.setStyleSheet(
            "color: #4CAF50; "
        ...
        self.port_checkbox = QCheckBox("Use custom RPC port:")
        self.port_input = QLineEdit()
        self.port_input.setPlaceholderText("8545 (default)")
        self.port_input.setMaximumWidth(100)
        self.port_input.setEnabled(False)
        self.port_checkbox.toggled.connect(self.port_input.setEnabled)
        
        port_layout.addWidget(self.port_checkbox)
        port_layout.addWidget(self.port_input)
        port_layout.addWidget(QLabel("(optional - leave unchecked for auto-selection)"))
        port_layout.addStretch()
        layout.addLayout(port_layout)
        
        self.registerField("custom_port", self.port_checkbox)
        self.registerField("port_value", self.port_input)
        
        layout.addStretch()
        self.setLayout(layout)
    
    def initializePage(self) -> None:
        """Initialize the page with network selection."""
        ...
        self.network_display.setText(network)
...
class FirstRunWizard(QWizard):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.addPage(LocalNodeConfigPage())  # Replaced RPCConfigPage
    ...
    def accept(self) -> None:
        ...
        try:
            ...
            # Network configuration - local node only
            if self.field("mainnet"):
            ...
            if self.field("custom_port"):
                port_text = self.field("port_value")
                if port_text:
                    try:
                        config.network.local_rpc_port = int(port_text)
                    except ValueError:
                        pass  # Ignore invalid port, will use auto
