...
class _CreateWalletDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._dilithium_desc = QLabel("Fast signing/verification and smaller signatures.")
        ...
        self._sphincs_desc = QLabel("Large signatures / slower.")
        ...
        self._error_label.setWordWrap(True)
        layout.addWidget(self._error_label)
        self._details_toggle = QCheckBox("Show command details")
        ...
        self._output_log.setMinimumHeight(100)
        ...
        diag_row.addStretch()
        diag_row.addWidget(self._copy_diag_btn)
    ...
    def _copy_diagnostics(self) -> None:
        ...
        QTimer.singleShot(1200, lambda: self._copy_diag_btn.setText("Copy diagnostics"))
    def set_cli_preview(self, args: list[str]) -> None:
        self._details_label.setText("animica " + " ".join(args))
    ...
    def set_busy(self, busy: bool, status: str = "Creating wallet…") -> None:
        self._is_busy = busy
...
class WalletPage(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._create_wallet_stdout: list[str] = []
        self._create_wallet_stderr: list[str] = []
        self._create_wallet_error = ""
        self._create_wallet_exit_code: int | None = None
        self._create_wallet_requested_label: str | None = None
        self._create_wallet_requested_scheme: str | None = None
        self._create_wallet_requested_args: list[str] | None = None
        self._build_ui()
    ...
    def _start_create_wallet(self, label: str, sig_scheme: str, allow_insecure_fallback: bool) -> None:
        ...
        if not qalive(self._create_wallet_dialog):
            return
        dialog = self._create_wallet_dialog
        dialog.set_error("")
        dialog.clear_output()
        try:
            create_args, clean_label, scheme = self._wallet_service.build_create_wallet_args(
                label,
                sig_scheme,
                allow_insecure_fallback=allow_insecure_fallback,
            )
            dialog.set_cli_preview(create_args)
            job, clean_label, scheme = self._wallet_service.start_create_wallet(
                self._runner,
                label,
            ...
            self._console_system_error(f"[error] wallet create start failed: {safe_str(exc)}")
            return
        self._create_wallet_job = job
        self._create_wallet_btn.setEnabled(False)
        dialog.set_busy(True)
        self._create_wallet_stdout = []
        self._create_wallet_stderr = []
        self._create_wallet_error = ""
        self._create_wallet_exit_code = None
        self._create_wallet_requested_label = clean_label
        self._create_wallet_requested_scheme = scheme
        self._create_wallet_requested_args = create_args
        job.output.connect(self._on_create_wallet_output)
        job.error.connect(self._on_create_wallet_error)
        job.finished.connect(self._on_create_wallet_finished)
    def _on_create_wallet_output(self, _job_id: str, stream: str, text: str) -> None:
        if self._create_wallet_job is None:
            return
        if stream == "stdout":
            self._create_wallet_stdout.append(text)
        elif stream == "stderr":
            self._create_wallet_stderr.append(text)
        if qalive(self._create_wallet_dialog):
            self._create_wallet_dialog.append_output(stream, text)

    def _on_create_wallet_error(self, _job_id: str, msg: str, details: str) -> None:
        rendered = msg if not details else f"{msg}\n{details}"
        self._create_wallet_error = rendered
        if qalive(self._create_wallet_dialog):
            self._create_wallet_dialog.set_error(rendered)
        self._console_system_error(f"[error] {rendered}")

    def _on_create_wallet_finished(self, _job_id: str, exit_code: int, _payload: object) -> None:
        self._create_wallet_exit_code = exit_code
        if exit_code != 0:
            stderr = "\n".join(s for s in self._create_wallet_stderr if s.strip()).strip()
            if stderr and qalive(self._create_wallet_dialog):
                self._create_wallet_dialog.set_error(stderr)
            self._finalize_create_wallet()
            return
        if qalive(self._create_wallet_dialog):
            self._create_wallet_dialog.set_busy(True, "Finalizing…")
        self._reload_wallets_after_create(attempt=1)

    def _reload_wallets_after_create(self, attempt: int) -> None:
        records = self._wallet_store.reload_local_wallets()
        if records or attempt >= 5:
            self._reload_accounts_from_source_safe()
            target_label = (self._create_wallet_requested_label or "").strip().lower()
            if target_label:
                for idx, acc in enumerate(self._display_accounts):
                    if acc.label.strip().lower() == target_label:
                        self._accounts_list.setCurrentRow(idx)
                        break
            if qalive(self._create_wallet_dialog):
                self._create_wallet_dialog.accept()
            QMessageBox.information(self, "Wallet", f"Wallet created: {self._create_wallet_requested_label}")
            self._finalize_create_wallet()
            return
        QTimer.singleShot(150, lambda: self._reload_wallets_after_create(attempt + 1))

    def _finalize_create_wallet(self) -> None:
        if qalive(self._create_wallet_dialog):
            self._create_wallet_dialog.set_diagnostics(self._build_create_wallet_diagnostics())
            self._create_wallet_dialog.set_busy(False)
        self._create_wallet_btn.setEnabled(True)
        self._create_wallet_job = None

    def _build_create_wallet_diagnostics(self) -> str:
        wallets_path = self._wallets_path()
        exists = wallets_path.exists()
        stat_text = "missing"
        if exists:
            st = wallets_path.stat()
            stat_text = f"size={st.st_size},mtime={int(st.st_mtime)}"
        return "\n".join([
            f"argv: animica {' '.join(self._create_wallet_requested_args or [])}",
            f"exit_code: {self._create_wallet_exit_code}",
            f"stdout: {' '.join(self._create_wallet_stdout)[:1200]}",
            f"stderr: {' '.join(self._create_wallet_stderr)[:1200]}",
            f"error: {self._create_wallet_error}",
            f"wallets_json: {wallets_path}",
            f"wallets_json_exists: {exists}",
            f"wallets_json_info: {stat_text}",
        ])

    def _console_system_error(self, text: str) -> None:
        try:
            self._history_tab._console.append_system_line(text)
        except Exception:
            log.error(text)
