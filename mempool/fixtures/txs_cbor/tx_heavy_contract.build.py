...
def to_cbor(x)->bytes:
    ...
    if isinstance(x,int): return cbor_uint(x) if x>=0 else None  # not needed
...
p = pathlib.Path(os.path.expanduser("~/animica/mempool/fixtures/txs_cbor/tx_heavy_contract.cbor"))
p.write_bytes(out)
