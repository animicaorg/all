...
if (
    ...
    or validate_proposal.Draft202012Validator is None
):
    pytest.skip(
...
def test_example_md_files_validate_with_strict_bounds(
    md_file: Path, capsys: pytest.CaptureFixture
) -> None:
    """
    ...
    rc = validate_proposal.main(
        [
            ...
            str(SCHEMAS),
            "--bounds",
            str(REGISTRIES / "params_bounds.json"),
            "--strict",
        ]
    )
...
def test_sample_ballots_pass_quorum_and_approval(capsys: pytest.CaptureFixture) -> None:
    ...
    assert (
        report["evaluation"]["outcome"] == "passed"
    ), f"Outcome not passed:\n{json.dumps(report, indent=2)}"
    ...
    assert float(report["metrics"]["participationPercent"]) == pytest.approx(
        100.0, rel=0, abs=1e-6
    )
    assert float(report["metrics"]["approvalPercent"]) == pytest.approx(
        100.0, rel=0, abs=1e-6
    )
