...
const checkBitGo = () => {
  if (!env.BITGO_ACCESS_TOKEN || !env.BITGO_BASE_URL) {
    logger.warn("BitGo not configured; skipping connectivity check");
    return;
  }
  logger.info(
    {
      env: env.BITGO_ENV,
      baseUrl: env.BITGO_BASE_URL
    },
    "BitGo connectivity check stub"
  );
};
...
const start = async () => {
  ...
  const server = app.listen(env.PORT, "0.0.0.0", () => {
    logger.info({ port: env.PORT }, "wallet-router listening");
  });
...
start().catch((error) => {
  logger.error({ error }, "failed to start wallet-router");
  process.exit(1);
});
