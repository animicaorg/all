...
def verify(pubkey_hex: bytes | str, message: bytes, sig_hex: bytes | str) -> bool:
    ...
    if isinstance(pubkey_hex, bytes):
        try:
            pubkey = bytes.fromhex(pubkey_hex.decode() if all(32 <= b <= 127 for b in pubkey_hex) else pubkey_hex.decode('utf8'))
        except Exception:
            pubkey = pubkey_hex
    else:
    ...
    if isinstance(sig_hex, bytes):
        sig = bytes.fromhex(sig_hex.decode() if all(32 <= b <= 127 for b in sig_hex) else sig_hex.decode('utf8'))
    else:
