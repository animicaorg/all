...
from . import create as _reg_create, owner as _reg_owner  # type: ignore
...
def register_name(id_: bytes, name32: bytes, addr: bytes, set_primary: int = 1) -> int:
    ...
    if not was_present:
        ...
        if old_addr != addr:
            ...
            if cur_nm == name32:
                ...
                events.emit(b"NR:PrimaryCleared", {
                    b"id": id_,
                    b"addr": old_addr,
                })
    ...
    if set_primary != 0:
        ...
        events.emit(b"NR:PrimarySet", {
            b"id": id_,
            b"addr": bytes(addr),
            b"name": bytes(name32),
        })

    events.emit(b"NR:NameRegistered", {
        b"id":   id_,
        b"name": bytes(name32),
        b"addr": bytes(addr),
...
__all__ = [
    "create_registry", "id_from",
    "has_name", "resolve",
    "reverse_primary",
    "register_name", "clear_name",
