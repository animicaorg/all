...
from . import bool_flag, ensure_len, event_fields, to_bytes
...
def emit_ownership_transferred(
    previous_owner: Union[bytes, bytearray], new_owner: Union[bytes, bytearray]
) -> None:
...
def emit_role_granted(
    role: Union[bytes, bytearray],
    account: Union[bytes, bytearray],
    sender: Union[bytes, bytearray],
) -> None:
...
def emit_transfer(
    sender: Union[bytes, bytearray], to: Union[bytes, bytearray], value: int
) -> None:
    ensure_len(sender, min_len=1, max_len=64)
...
def emit_approval(
    owner: Union[bytes, bytearray], spender: Union[bytes, bytearray], value: int
) -> None:
...
def emit_escrow_deposited(
    escrow_id: Union[bytes, bytearray],
    payer: Union[bytes, bytearray],
    payee: Union[bytes, bytearray],
    amount: int,
) -> None:
...
def emit_escrow_released(
    escrow_id: Union[bytes, bytearray], to: Union[bytes, bytearray], amount: int
) -> None:
    ensure_len(escrow_id, min_len=1, max_len=64)
    ensure_len(to, min_len=1, max_len=64)
    _emit(
...
def emit_escrow_resolved(
    escrow_id: Union[bytes, bytearray], winner: Union[bytes, bytearray]
) -> None:
    ensure_len(escrow_id, min_len=1, max_len=64)
...
def emit_splitter_payment(
    release_id: Union[bytes, bytearray], to: Union[bytes, bytearray], amount: int
) -> None:
...
def emit_name_registered(
    name: Union[bytes, bytearray], owner: Union[bytes, bytearray]
) -> None:
...
def emit_name_updated(
    name: Union[bytes, bytearray],
    old_addr: Union[bytes, bytearray],
    new_addr: Union[bytes, bytearray],
) -> None:
...
def emit_ai_job_enqueued(
    task_id: Union[bytes, bytearray], model: Union[bytes, bytearray], fee: int
) -> None:
...
def emit_da_blob_pinned(
    ns: Union[bytes, bytearray, int],
    commitment: Union[bytes, bytearray],
    size_bytes: int,
) -> None:
    """
    ...
    if isinstance(ns, int):
        # minimal big-endian
        ns_b = (
            b"\x00" if ns == 0 else int(ns).to_bytes((ns.bit_length() + 7) // 8, "big")
        )
    else:
        ns_b = to_bytes(ns)
...
__all__ = [
    ...
    "EV_UNPAUSED",
    "EV_TIMELOCK_QUEUED",
    "EV_TIMELOCK_EXECUTED",
    "EV_TIMELOCK_CANCELED",
    "EV_TRANSFER",
    "EV_APPROVAL",
    "EV_MINT",
    "EV_BURN",
    ...
    "emit_paused",
    "emit_unpaused",
    "emit_timelock_queued",
    "emit_timelock_executed",
    "emit_timelock_canceled",
    "emit_transfer",
    "emit_approval",
    "emit_mint",
    "emit_burn",
    "emit_escrow_deposited",
    "emit_escrow_released",
    "emit_escrow_disputed",
    "emit_escrow_resolved",
    "emit_splitter_payment",
    "emit_name_registered",
    "emit_name_updated",
    "emit_ai_job_enqueued",
    "emit_ai_result_ready",
    "emit_quantum_job_enqueued",
    "emit_quantum_result_ready",
    "emit_da_blob_pinned",
    "emit_zk_verified",
    "emit_rand_committed",
    "emit_rand_revealed",
    "emit_rand_beacon_mixed",
]
