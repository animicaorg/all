...
@pytest.mark.parametrize("size", SMALL_SIZES + ([] if SKIP_HEAVY or is_ci() else HEAVY_SIZES))
...
def test_streaming_matches_one_shot_keccak(pattern: str, size: int):
    ...
    assert got == want, f"Keccak256 streaming != one-shot (pattern={pattern}, size={size})"
