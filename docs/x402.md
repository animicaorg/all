...
(default 0.0001 ETH ≈ 185 settlements of remaining runway), the module swaps
a little accrued USDC to ETH on Uniswap v3:
...
* **Pre-flight.** `eth_estimateGas` on the exact final calldata before
  ...
  skipped for free instead of burning gas to discover it. A sip is also
  skipped when the ETH it would buy is not worth ≥20× the gas it costs
  (today that ratio is ~2,000×, so tripping it means something is wrong).
...
The cold address must be **exactly EIP-55 checksummed** — an all-lowercase
address is rejected at startup. It is the one value where every swept dollar
ends up, and the checksum is the cheapest possible typo guard.
...
sandwiching. Total exposure is bounded by three independent caps: the sip
size ($5), one sip per cooldown (24 h), and the daily budget ($10). Worst
credible daily loss to slippage/MEV is **cents**.
...
### Funding guidance ($2-once bootstrap)

1. Send **~$2 of ETH** (≈0.001 ETH) to the facilitator address on Base. That
   is ~2,000 settlements of gas — far more than enough to reach the first
   sip.
2. Leave `X402_SETTLEMENT_ADDRESS` = the facilitator address,
   `X402_TREASURY_ENABLED=1`, `X402_TREASURY_COLD_ADDRESS` = your cold
   wallet. Everything else can stay at its default.
3. From then on: revenue accrues → surplus above $20 is swept to cold → when
   gas drops below 0.0001 ETH a $5 (or smaller) sip refuels it.
4. Watch `x402_treasury_sipping_enabled`, `x402_treasury_eth_balance_wei` and
   the `/readyz` warning. Alarm on the first two; the third is the operator's
   cue to top up manually.

You never need to send ETH again unless the breaker opens. If it does, the
facilitator keeps settling until the remaining ETH runs out — you have
roughly 185 settlements of warning from the floor, and `readyz` fails
outright at `X402_MIN_GAS_BALANCE_WEI` before the wallet is truly empty.
...
slippage bound or the economic sanity check. They also sign from the
facilitator wallet using its `pending` nonce, which the running service owns
behind a lock this process cannot see: stop the unit first, or be ready to
retry one of the two transactions.
...
`x402_treasury_sipping_enabled`, `x402_treasury_sweeping_enabled`. Treasury
