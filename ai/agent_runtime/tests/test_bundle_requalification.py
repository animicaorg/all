...
def test_failed_hf_download_keeps_partial_data_for_resume(tmp_path, monkeypatch):
    """A transient network error must not turn every retry into a full download."""
    import huggingface_hub
    from agent_runtime.aicf_worker import bootstrap_bundle_from_hf
    from agent_runtime.errors import BundleError

    cfg = SimpleNamespace(
        integration={"aicf": {"miner_cache_dir": str(tmp_path)}},
        model_catalog={
            "tiers": [{"id": "tiny", "base_model": "test/agent-2b",
                       "precision": "int4", "context_window": 4096}],
            "propagation": {"miner_cache_dir": str(tmp_path)},
        },
    )
    ...
    def interrupted(**kwargs):
        ...
        (model_dir / "model-part-1.safetensors").write_bytes(b"partial")
        raise OSError("connection reset")

    monkeypatch.setattr(huggingface_hub, "snapshot_download", interrupted)
    with pytest.raises(BundleError):
        bootstrap_bundle_from_hf("tiny", cfg=cfg)

    bundle = tmp_path / "tiny" / "hf-test__agent-2b"
    assert (bundle / "model" / "model-part-1.safetensors").is_file()
    assert not (bundle / "manifest.json").exists()

    def resumed(**kwargs):
        model_dir = Path(kwargs["local_dir"])
        assert (model_dir / "model-part-1.safetensors").is_file()
        (model_dir / "config.json").write_text("{}", encoding="utf-8")

    monkeypatch.setattr(huggingface_hub, "snapshot_download", resumed)
    completed = bootstrap_bundle_from_hf("tiny", cfg=cfg)
    assert completed == bundle
    assert (bundle / "manifest.json").is_file()
    assert (bundle / "inference.json").is_file()
