...
def test_explorer_api_endpoints_return_sane_aggregates():
    base = _api_base()
    if not base:
        pytest.skip("Set ANIMICA_EXPLORER_API_URL (or EXPLORER_API_URL) to run explorer-web API checks.")

    # 1) Summary endpoint — head & basic counters
    sum_path, summary = _try_paths(base, [
        "/summary",
        "/api/summary",
        "/v1/summary",
        "/explorer/summary",
    ...
    head_h = summary.get("headHeight") or summary.get("height") or summary.get("head_height")
    head_hash = summary.get("headHash") or summary.get("hash") or summary.get("head_hash")

    # Minimal sanity: we should get at least height as an integer >= 0
    hh_i = _as_int(head_h)
    assert hh_i is not None and hh_i >= 0, f"Invalid head height in {sum_path}: {head_h!r}"

    if chain_id is not None:
        cid_i = _as_int(chain_id)
        assert cid_i is not None and cid_i >= 0, f"Invalid chainId in {sum_path}: {chain_id!r}"

    if head_hash is not None:
        assert _is_hex(head_hash), f"headHash is not hex string in {sum_path}: {head_hash!r}"
    ...
    for k in ("txCount", "txs", "blockCount", "blocks", "pendingTxs", "pending"):
        ...
        if v is not None:
            vi = _as_int(v)
            assert vi is not None and vi >= 0, f"Counter {k} invalid in {sum_path}: {v!r}"

    # 2) Blocks list — most recent N and monotonic heights
    blk_path, blks = _try_paths(base, [
        "/blocks?limit=5",
        "/api/blocks?limit=5",
        "/v1/blocks?limit=5",
        ...
        if isinstance(items, list) and items:
            ...
            for b in items:
                if isinstance(b, dict):
                    ...
                    if hsh is not None:
                        assert _is_hex(hsh), f"Block hash not hex in {blk_path}: {hsh!r}"
    ...
    if po:
        ...
        if acc is not None:
            a = float(acc) if isinstance(acc, (int, float)) else float(_as_int(acc) or 0)
            assert 0.0 <= a <= 1.0, f"Accept rate out of range in {po_path}: {acc!r}"
        gamma = po.get("Gamma") or po.get("gamma") or po.get("totalGamma") or po.get("total_gamma")
        ...
        if isinstance(shares, dict):
            for typ, val in shares.items():
                vi = _as_int(val)
                assert vi is not None and vi >= 0, f"sharesByType[{typ}] invalid in {po_path}: {val!r}"

    # 4) AICF aggregates — providers/jobs/throughput (if present)
    aicf_path, aicf = _try_paths(base, [
        "/aicf",
        "/aicf/metrics",
        "/api/aicf/summary",
        "/v1/aicf/summary",
    ...
    if aicf:
        for k in ("providersOnline", "providers", "jobsQueued", "jobsPending", "jobsCompleted"):
            v = aicf.get(k)
            if v is not None:
                vi = _as_int(v)
                assert vi is not None and vi >= 0, f"AICF metric {k} invalid in {aicf_path}: {v!r}"

    # 5) DA aggregates — blobs/commitments/availability (if present)
    da_path, da = _try_paths(base, [
        "/da",
        "/da/summary",
        "/api/da/summary",
        "/v1/da/summary",
    ...
    if da:
        for k in ("blobs", "commitments", "proofsServed", "proofs", "samplePFail", "p_fail"):
            v = da.get(k)
            if v is None:
                continue
            if "fail" in k.lower():
                ...
                try:
                    ...
                    assert 0.0 <= f <= 1.0, f"DA metric {k} out of [0,1] in {da_path}: {v!r}"
                except Exception:
                    pytest.fail(f"DA metric {k} not a number in {da_path}: {v!r}")
            else:
                vi = _as_int(v)
                assert vi is not None and vi >= 0, f"DA metric {k} invalid in {da_path}: {v!r}"

    # 6) Randomness / beacon — round and beacon present if exposed
    rnd_path, rnd = _try_paths(base, [
        "/randomness",
        "/randomness/summary",
    ...
    if rnd:
        ...
        if r_id is not None:
            ri = _as_int(r_id)
            assert ri is not None and ri >= 0, f"Beacon round invalid in {rnd_path}: {r_id!r}"
