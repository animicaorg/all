...
async def _try_get(
    client: httpx.AsyncClient, base: str, p: Paths, commitment: str, state: SoakState
) -> None:
        ...
        if r.status_code == 200:
            ...
            if commitment:
                ...
                if do_get_prob > 0.0:
                    ...
                    if (
                        hash((rpc_id * 1103515245 + 12345)) & 0xFFFF
                    ) / 65535.0 < do_get_prob:
                        await _try_get(client, base, p, commitment, state)
                # Optional PROOF
                if do_proof_samples > 0:
                    await _try_proof(
                        client, base, p, commitment, do_proof_samples, state
                    )
        else:
            state.posted_err += 1
    except Exception:
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Steady DA POST load with optional availability sampling and GET verification."
    )
    ap.add_argument(
        "--base",
        default=os.environ.get("DA_URL", "http://127.0.0.1:8666"),
        help="Base URL for DA service (default: %(default)s or DA_URL env)",
    )
    ap.add_argument(
        "--post-path", default="/da/blob", help="POST path (default: /da/blob)"
    )
    ap.add_argument(
        "--get-path",
        default="/da/blob/{commitment}",
        help="GET path template (default: /da/blob/{commitment})",
    )
    ap.add_argument(
        "--proof-path",
        default="/da/proof",
        help="Proof path (supports {commitment} or ?commitment=)",
    )

    ap.add_argument(
        "--rps", type=float, default=10.0, help="Target POSTs per second (default: 10)"
    )
    ap.add_argument(
        "--duration", type=float, default=60.0, help="Duration seconds (default: 60)"
    )
    ap.add_argument(
        "--max-inflight",
        type=int,
        default=200,
        help="Max concurrent in-flight requests (default: 200)",
    )

    ap.add_argument(
        "--min-kib", type=int, default=64, help="Minimum blob size KiB (default: 64)"
    )
    ap.add_argument(
        "--max-kib", type=int, default=512, help="Maximum blob size KiB (default: 512)"
    )

    ap.add_argument(
        "--ns-min", type=int, default=16, help="Minimum namespace id (default: 16)"
    )
    ap.add_argument(
        "--ns-max", type=int, default=24, help="Maximum namespace id (default: 24)"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=424242,
        help="Deterministic payload seed (default: 424242)",
    )
