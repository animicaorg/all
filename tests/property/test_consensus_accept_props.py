...
def _call_accept(psi_sum: float, theta: float, u: Optional[float]) -> bool:
    ...
    try:
        ...
        takes_u = any(
            p for p in params.values() if p.name in ("u", "uniform", "rand_u")
        )
    except Exception:
        takes_u = False
...
@pytest.mark.skipif(
    not _has_accept(), reason="consensus.scorer acceptance predicate not available"
)
@given(THETA, DELTA)
...
def test_accept_probability_matches_theory(theta: float, delta: float):
    ...
    assert (
        abs(p_emp - p_exp) <= 0.07
    ), f"empirical={p_emp:.4f} vs expected={p_exp:.4f} (Θ={theta:.3f}, ψ={psi_sum:.3f})"
...
def test_monotone_in_psi(theta: float, psi_base: float, bump: float):
    ...
    assert (
        p_high + 0.02 >= p_low
    ), f"p_high={p_high:.3f} < p_low={p_low:.3f} (Θ={theta:.3f})"
...
def test_accept_always_when_psi_ge_theta(theta: float, extra: float):
    ...
    assert all(
        results
    ), f"Found rejection despite ψ>=Θ (Θ={theta:.4f}, ψ={psi_sum:.4f})"
...
def test_accept_rare_when_theta_far_above_psi(theta: float, delta_big: float):
    ...
    assert (
        p_emp <= 0.01
    ), f"Acceptance too high for large Δ (emp={p_emp:.4f}, Δ={delta_big:.3f})"
