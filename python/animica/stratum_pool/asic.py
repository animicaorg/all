...
class Sha256RpcAdapter:
    def __init__(self, rpc_url: str, pool_address: str, *, logger: Optional[logging.Logger] = None) -> None:
    ...
    async def get_new_job(self) -> Sha256Job:
        payload = await self._rpc_call("miner.get_sha256_job", [
        ...
        return Sha256Job(
            ...
            target=int(payload.get("target"), 16) if payload.get("target") is not None else _bits_to_target("1d00ffff"),
...
class Sha256ShareValidator:
    ...
    def _header_hash(self, job: Sha256Job, merkle_root: bytes, ntime_hex: str, nonce_hex: str) -> bytes:
        import struct
    ...
    def validate(self, job: Sha256Job, session: "Sha256Session", submit_params: List[Any]) -> tuple[bool, Optional[str], bool]:
        try:
...
class Sha256Session:
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            "last_share_status": "accepted" if self.shares_accepted else "rejected" if self.shares_rejected else None,
...
class Sha256StratumServer:
    def __init__(
        ...
        *,
        ...
        self._submit_hook: Optional[
            Callable[[Sha256Session, Sha256Job, Dict[str, Any], bool, Optional[str], bool, int], Awaitable[None]]
        ] = None
        self._accepted = 0
        self._rejected = 0
    ...
    async def _process_message(self, session: Sha256Session, msg: Dict[str, Any]) -> None:
        ...
        if method == "mining.subscribe":
            ...
            await self._send(session.writer, {"id": msg_id, "result": result, "error": None})
            ...
            if self._submit_hook:
                await self._submit_hook(session, job, submit_payload, accepted, reason, is_block, 0)

        elif method == "mining.configure":
            await self._send(session.writer, {"id": msg_id, "result": {}, "error": None})

        else:
            await self._send(session.writer, {"id": msg_id, "result": None, "error": "unknown method"})
...
class _Sha256JobManager:
    def __init__(self, adapter: Sha256RpcAdapter, on_job: Callable[[Sha256Job], Awaitable[None]]) -> None:
        self._adapter = adapter
        self._on_job = on_job
