...
# models (Qwen 0.5B, etc.) have at least factual grounding about
# Animica instead of hallucinating a "mobile games division". Keep it
# short — every token here is a token the model has to read before
# answering. Override with ANIMICA_AICF_SYSTEM_PROMPT to customize
# (set to empty string to disable).
_DEFAULT_SYSTEM_PROMPT = (
    "You are an AI assistant served by an Animica AICF (AI Compute Fund) "
    "worker. Animica is a Layer-1 blockchain with built-in paid AI "
    "inference; native token is ANIMICA. Mining is hashshare PoW, and "
    "miners may additionally serve AICF jobs to earn ANIMICA per chat "
    "turn. Public mainnet pool: pool.animica.org (Stratum on TCP 3333). "
    "This very reply was generated by a miner running real model "
    "inference on the Animica network. "
    "Answer the user's question directly. If you don't know an "
    "Animica-specific detail, say so plainly rather than guessing."
)
